...
int32_t gimbal_set_offset(struct gimbal* gimbal, uint16_t yaw_ecd, uint16_t pitch_ecd)
{
    ...
    gimbal->param.yaw_ecd_center = 2670;//HERO-RM
...
int32_t gimbal_cascade_calculate(struct gimbal* gimbal)
{
	...
	//HERO_RM
	if(MiniPC_CMD)
		motor_out = pid_calculate(&(gimbal->yaw_inter_pid), gimbal->sensor.rate.yaw_rate, my_yaw_speed);
	else
